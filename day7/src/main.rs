...
impl FileSystem {
    fn new() -> Self {
        Self { 
            root: DirectoryInfo { name: "<root>".to_string(),  files: Vec::new(), directories: Vec::new()}
        }
    }
...
fn riddle_part_one(file_path: &String)  {
    ...
    let mut all_paths : HashSet<FileSystemPath>= HashSet::new();
        ...
        if command.starts_with("ls") {
            ...
            for response in response_lines {
                if response.starts_with("dir ") {
                    ...
                    let dir = DirectoryInfo { name: dir_name.to_string(), directories: Vec::new(), files: Vec::new() };
                    dirs.push(dir);
                    
                    let mut new_dir = cwd.clone();
                    new_dir.push(dir_name.to_string());
                    all_paths.insert(new_dir);
                }else {
    ...
    //println!("DUMPING FS: {:?}", fs);

    let mut path_size_map : HashMap<FileSystemPath, usize> = HashMap::new();
    for path in all_paths.iter() {
        if let Some(directory)  = fs.mut_directory_by_path(path) {
            let size = directory.recursive_size();
            path_size_map.insert(path.clone(), size);
        }
    }

    let sum_dirs_beneath_size : usize = path_size_map.iter_mut()
